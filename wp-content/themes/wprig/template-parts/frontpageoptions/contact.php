...
/**
 * TO DO
 * This needs to be more effective.
 */
function acknowledgeFormFill() {
	const acknowledgement = document.createElement( 'div' );
	acknowledgement.setAttribute( 'class', 'acknowledgement' );
}
...
let onComplete = function( formData ) {
	...
	fetch( sheetInfo.googleWebApp, { method: 'POST', body: newData } )
		...
		.then( response => {
			...
			h1.classList.add( 'acknowledgement' );
			...
			closeIcon.setAttribute('class', 'material-icons light-text close-acknowledgement' );
			closeIcon.setAttribute('title', 'click to contact form submission acknowledgement' );
			closeIcon.setAttribute('id', 'closeAcknowledgement' );
