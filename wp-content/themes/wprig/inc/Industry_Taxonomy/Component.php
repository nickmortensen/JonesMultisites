<?php
/**
 * WP_Rig\WP_Rig\Industry_Taxonomy\Component class
 *
 * @package wp_rig
 */

namespace WP_Rig\WP_Rig\Industry_Taxonomy;
...
class Component implements Component_Interface, Templating_Component_Interface {
	 ...
	 * @param string $taxonomy The taxonomy term. Default to 'industry'.
	 *
	 * @return array $info An associative array of all the data and metadata assigned to thie taxonomy item.
	 */
	public function get_all_industry_info( $term_id = '', $taxonomy = 'industry' ) : array {
		$info      = [];
		$term      = get_term( $term_id, $taxonomy );
		$alternate = $this->get_industry_alternate_terms( $term_id );
		return $info;
	}
	 ...
	 *
	 * @param int    $term_id The id for the taxonomy term.
	 * @param string $field The termmeta field.
	 * @param bool   $single Whether we should output the first item only (true), or the entire array(false). Defaults to false.
	 * @return array $info An associative array of all the data and metadata assigned to thie taxonomy item.
	 */
	public function get_industry_alternate_terms( $term_id, $field = 'industryAltNames', $single = false ) {
		return get_term_meta( $term_id, $field, $single );
	}
	 ...
	 */
	public function get_industry_ids() : array {
		$ids       = [];
		$items = $this->get_all_industries();
	...
	/**
	 * Retrieve the taxonomy meta for 'industryAltNames' for the given sign type.
	 ...
	 * @return mixed if $id is true, retrieve the (int) id of the image, otherwise retrieve the url as a string.
	 */
	public function get_all_industry_images( $term_id, $id = true ) : array {
		$output = [];
	 ...
	 */
	public function get_service_indepth( $term_id ) : string {
		$prefix = $this->get_slug();
		$key    = $prefix . 'Indepth';
		$single = true;
		$output = get_term_meta( $term_id, $key, $single );
...
JSONLD;
	...
	/**
	 * Put Rich Snippet for the industry in the head.
	 */
