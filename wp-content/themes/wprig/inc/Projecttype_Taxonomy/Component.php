<?php
/**
 * WP_Rig\WP_Rig\Projecttype_Taxonomy\Component class
 *
 * @package wp_rig
 */

namespace WP_Rig\WP_Rig\Projecttype_Taxonomy;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function template_tags() : array {
		return [
			'get_all_projecttype_info'        => [ $this, 'get_all_projecttype_info' ],
		];
	}
	 ...
	 */
	public function get_all_projecttype_info( $term_id ) : array {
		$taxonomy                = 'projecttype';
		$info                    = [];
		$projecttype             = get_term( $term_id, $taxonomy );
		$info['name']            = $projecttype->name;
		$info['term_id']         = $term_id;
		$info['description']     = $projecttype->description;
		$info['aliases']         = $this->get_aliases( $term_id );
		$info['indepth']         = $this->get_projecttype_indepth( $term_id );
		return $info;
	}
	 ...
	 */
	public function get_all_projecttypes() : array {
		$terms = get_terms( $this->get_slug(), [ 'hide_empty' => false ] );
		return $terms;
	}

	/**
	 * Get all projecttype term identifiers.
	 */
	public function get_projecttype_ids() : array {
		$ids   = [];
		$types = $this->get_all_projecttypes();
	 ...
	 */
	public function get_links( $except = [] ) : string {
		$types = $this->get_projecttype_ids();
		$links = [];
		foreach ( $types as $type ) {
	 ...
	 *
	 * Use CMB2 to create additional fields for the custom taxonomy.
	 *
	 * @since  1.0.0
	...
	public function create_extra_fields() {
		...
		$args    = [
			'id'           => $prefix . 'edit',
			'title'        => 'Project Type Additional Info',
			'object_types' => [ 'term' ],
			'taxonomies'   => [ $prefix ],
			'cmb_styles'   => false,
	 ...
	 */
	public function set_projecttype_admin_columns( $columns ) {
	 ...
	 * @return mixed if $id is true, retrieve the (int) id of the image, otherwise retrieve the url as a string.
	 */
	public function get_all_images( $term_id, $id = true ) : array {
		$output                = [];
	...
	/**
	 * Retrieve the taxonomy meta for 'projecttypeIndepth' for the given project type.
	 * This is always an string.
	 *
	 * @param int $term_id Signtype Taxonomy Id.
	 * @return array The sudomain of this location's homepage.
	 */
	public function get_projecttype_indepth( $term_id ) : string {
		$prefix = $this->get_slug();
		$key    = $prefix . 'Indepth';
		$single = true;
		$output = get_term_meta( $term_id, $key, $single );
	 ...
	 * @link https://developers.google.com/search/docs/data-types/product
	 */
	private function get_rich_snippet( $taxonomy = 'projecttype' ) {
		$term_id  = get_queried_object()->term_id;
		$term     = get_term( $term_id, $taxonomy );
		$slug     = $term->slug;
		$desc     = $term->description;
		$name     = $term->name;
		$info     = $this->get_all_projectttype_info( $term_id );
...
JSONLD;
	...
	/**
	 * Put Rich Snippet for the taxonomy term in the head.
	 */
	public function add_projecttype_rich_snippet_to_head() {
		if ( is_tax( 'projecttype' ) ) {
...
}//end class Projecttype_Taxonomy
